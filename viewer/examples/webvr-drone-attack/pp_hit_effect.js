...
var krpanoplugin = function () {
  ...
  local.registerplugin = function (krpanointerface, pluginpath, pluginobject) {
    ...
    if (krpano.webGL) {
      shader = krpano.webGL.createPostProcessingShader(
        effect_shader_src(),
        "effect",
        "pp_hit_effect"
      );
      ...
      plugin.registerattribute("order", xml_order, xml_order_set, function () {
        return xml_order;
      });
      plugin.registerattribute("phase", xml_phase, xml_phase_set, function () {
        return xml_phase;
      });
  ...
  function BOOLEAN(value) {
    return typeof value === "boolean"
      ? value
      : ".yes.on.true.1.".indexOf(("." + value + ".").toLowerCase()) >= 0;
  }
  ...
  function xml_effect_set(v) {
    validate(v, "number", xml_effect, function (n) {
      xml_effect = n;
      request_shader_update();
    });
  }
  function xml_order_set(v) {
    validate(v, "lowstr", xml_order, function (s) {
      xml_order = s;
      STATE_shader_order_changed = true;
      request_shader_update();
    });
  }
  function xml_phase_set(v) {
    validate(v, "int", xml_phase, function (i) {
      xml_phase = i;
      STATE_shader_order_changed = true;
      request_shader_update();
    });
  ...
  function add_shaders() {
    if (STATE_shader_added == false) {
      ...
      sortorder = xml_order = validate_unique_sortorder(
        targetShaderArray,
        sortorder
      );
